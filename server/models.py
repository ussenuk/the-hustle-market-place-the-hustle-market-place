...
class ServiceProvider(db.Model):
    __tablename__ = 'service_provider'  # Explicitly specify table name
    id = db.Column(db.Integer, primary_key=True)
    fullname = db.Column(db.String(255))
    username = db.Column(db.String(255), unique=True)
    email = db.Column(db.String(255), unique=True)
...
class Service(db.Model):
    __tablename__ = 'service'  # Explicitly specify table name
    id = db.Column(db.Integer, primary_key=True)
    service_title = db.Column(db.String(255))
    service_category = db.Column(db.String(255))
...
class Booking(db.Model):
    __tablename__ = 'booking'  # Explicitly specify table name
    id = db.Column(db.Integer, primary_key=True)
    service_provider_id = db.Column(db.Integer, db.ForeignKey('service_provider.id'))
...
class Review(db.Model):
    __tablename__ = 'review'  # Explicitly specify table name
    id = db.Column(db.Integer, primary_key=True)
...
class Payment(db.Model):
    __tablename__ = 'payment'  # Explicitly specify table name
    id = db.Column(db.Integer, primary_key=True)
